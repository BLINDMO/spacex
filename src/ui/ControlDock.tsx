...
export default function ControlDock() {
  ...
  if (!sim) return null;
  const fm = sim.flightMode;
  const autoThrottle = fm === 'auto';
  const autoPitch = fm === 'assist' || fm === 'auto';
  const thr = autoThrottle ? sim.throttle : sim.manualThrottle;
  const pitchDeg = (autoPitch ? sim.bodyPitch : sim.manualPitch) * RAD;
  const warps: TimeWarp[] = [1, 5, 20, 100];
  const cams: CameraMode[] = ['follow', 'pad', 'orbit'];
  const modes: ('assist' | 'guided' | 'manual')[] = ['assist', 'guided', 'manual'];
  // staging readiness: highlight STAGE when the active stage is nearly dry
  const activeInit = runtime && sim.activeStage < runtime.stages.length
    ? (runtime.stages[sim.activeStage] as { propInitial: number }).propInitial : 0;
  const activeRem = sim.stagePropRemaining[sim.activeStage] ?? 0;
  const stageReady = activeInit > 0 && activeRem / activeInit < 0.06 && sim.activeStage < (runtime?.stages.length ?? 0);
  ...
  return (
    <div className="bg-panel/90 border-t border-edge px-2 pt-1.5 pb-[max(0.5rem,env(safe-area-inset-bottom))] backdrop-blur-sm">
      ...
      <div className="mb-1.5 flex gap-1">
        {modes.map((m) => (
          <button
            ...
            className={`dock-btn flex-1 ${fm === m ? 'border-accent text-accent' : ''}`}
          >
            {m.toUpperCase()}
          </button>
        ))}
      ...
      {/* Throttle (player-controlled in all skill modes) */}
      <div className="mb-1.5">
        <div className="flex items-center justify-between text-[10px] text-dim mb-0.5">
          ...
          <span className="flex items-center gap-2">
            {fm === 'assist' && <span className="text-dim">suggest {pct(sim.suggestedThrottle, 0)}</span>}
            <span className={`tnum text-sm ${autoThrottle ? 'text-dim' : 'text-accent'}`}>{pct(thr, 0)}</span>
          </span>
        </div>
        <input
      ...
      {/* Pitch (manual in guided/manual; autopilot flies it in assist) */}
      <div className={`mb-1.5 ${autoPitch ? 'opacity-40' : ''}`}>
        <div className="flex items-center justify-between text-[10px] text-dim mb-0.5">
          <span className="tracking-[0.15em]">PITCH {autoPitch ? '(auto)' : `· target ${num(sim.targetPitch * RAD, 0)}°`}</span>
          <span className="tnum text-sm text-ink">{num(pitchDeg, 0)}°</span>
        </div>
        <div className="flex items-center gap-2">
          <button disabled={autoPitch} onClick={() => setPitch(Math.max(0, sim.manualPitch - 5 * DEG))} className="dock-btn w-12 text-base">−</button>
          <input
          ...
          />
          <button disabled={autoPitch} onClick={() => setPitch(Math.min(90 * DEG, sim.manualPitch + 5 * DEG))} className="dock-btn w-12 text-base">+</button>
        </div>
      </div>

      {/* Sequencing actions */}
      <div className="grid grid-cols-4 gap-1.5">
        <button onClick={stageNow} className={`dock-btn ${stageReady ? 'border-go text-go animate-pulse' : ''}`}>STAGE</button>
